...
test.describe('Inbox · smoke', () => {
  ...
  test('ESC-5: action items arriba, waiting abajo, con contadores', async ({
    ...
    try {
      ...
      const waitingSection = page.locator(
      ...
      await expect(actionSection.locator('h2 + span')).toHaveText('3')
      await expect(waitingSection.locator('h2 + span')).toHaveText('2')

      await expect(actionSection.getByRole('listitem')).toHaveCount(3)
      await expect(waitingSection.getByRole('listitem')).toHaveCount(2)
    } finally {
      await resetInbox(workspaceId!)
    }
  })
  ...
  test('ESC-22: mark as read elimina el item de la lista', async ({
    ...
    try {
      ...
      await expect(actionSection.getByRole('listitem')).toHaveCount(1)

      await actionSection
        .getByRole('button', { name: /marcar item como leído/i })
        .click()

      await expect(actionSection.getByRole('listitem')).toHaveCount(0)
    } finally {
  ...
  test('ESC-28: empty state unificado cuando no hay items', async ({
    ...
    await expect(
      page.locator('section[aria-labelledby="inbox-empty-title"]'),
    ).toBeVisible()
    await expect(page.locator('#inbox-empty-title')).toBeVisible()

    await expect(
      page.locator('section[aria-labelledby="action-title"]'),
    ).toHaveCount(0)
    await expect(
      page.locator('section[aria-labelledby="waiting-title"]'),
    ).toHaveCount(0)
  })
