...
import { createOfferBonusTermsSchemas } from './bonusTerms'

export function createBundleEditorSchemas() {
  const { offerBonusTermsFormSchema } = createOfferBonusTermsSchemas()
  ...
  const bundleDeliverableSchema = z.object({
    ...
    amount: z
      .string()
      .refine((val) => val.length === 0 || parseFloat(val) > 0, {
        message: t`Amount must be greater than 0`,
      }),
  })

  const bundleEditorBaseSchema = z.object({
  ...
  const bundleEditorSubmitSchema = bundleEditorBaseSchema.superRefine(
    (data, ctx) => {
  ...
  return { bundleEditorBaseSchema, bundleEditorSubmitSchema }
}
