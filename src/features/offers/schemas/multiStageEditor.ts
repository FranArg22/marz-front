...
export function createMultiStageEditorSchemas() {
  ...
  const multiStageEditorBaseSchema = z.object({
    campaign_id: z.string().min(1, t`Select a campaign`),
    stages: z.array(stageSchema).min(2, t`Minimum 2 stages`),
  })

  const multiStageEditorSubmitSchema = multiStageEditorBaseSchema.superRefine(
    (data, ctx) => {
  ...
  return { multiStageEditorBaseSchema, multiStageEditorSubmitSchema }
}
