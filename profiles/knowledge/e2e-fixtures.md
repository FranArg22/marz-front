...
| `provisionTestSubscription` | `POST /v1/test/billing/subscription`                  | Sí                  | Pone un brand workspace en plan pago **real** (Stripe test: customer + `pm_card_visa` + subscription) → charge-on-send funciona. Lo usa `seed` para que la brand arranque paga. Body: `brand_workspace_id`, opcionales `plan` (default `growth`)/`interval`/`email`. Handler raw (no generated client; se llama con `fetch`). |
